...
function createSiteList(result,callback)
{
	...
	for(i=0;i<datas.length;i++)
	{
        ...
        // for (j in datas[i])
        // {
        //     params[j]=escape(datas[i]['id']);
        // }
        //  alert(JSON.stringify(params));
        var params={sid:datas[i].Sid,sitename:escape(datas[i].Site_Name)};
		// alert(JSON.stringify(params));
		rowHtml +='<li class="aui-list-item" tapmode onclick=openWin("editsite",'+JSON.stringify(params)+')>';
